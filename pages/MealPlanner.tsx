...
const MealPlanner: React.FC = () => {
  ...
  const [recipeBank, setRecipeBank] = useState<PlanEntry[]>(() => {
    ...
    if (locState?.addedRecipe) {
      const exists = RECIPE_BANK.find(
        (r) => r.name === locState.addedRecipe!.name,
      );
      if (!exists) {
  ...
  const [weekPlans, setWeekPlans] = useState<Record<number, PlanData>>({
    0: INITIAL_PLAN,
  });
  const plan: PlanData =
  ...
  const dropOnCell = (day: string, meal: string) => {
    ...
    setWeekPlans((prev) => ({
      ...prev,
      [weekOffset]: { ...plan, [day]: { ...plan[day], [meal]: dragging } },
    }));
    setDragging(null);
    setHoverCell(null);
  };
  ...
  const removeEntry = (e: React.MouseEvent, day: string, meal: string) => {
    ...
    setWeekPlans((prev) => ({
      ...prev,
      [weekOffset]: { ...plan, [day]: { ...plan[day], [meal]: null } },
    }));
  };
  ...
  const pickRecipe = (recipe: PlanEntry) => {
    ...
    setWeekPlans((prev) => ({
      ...prev,
      [weekOffset]: { ...plan, [day]: { ...plan[day], [meal]: recipe } },
    }));
    setPickerCell(null);
  };
                ...
                {DAYS.map((day, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {entry ? (
                        <div
                          className="relative group rounded-xl overflow-hidden border border-[#334155] cursor-pointer"
                        ...
                        >
                          ...
                          />
                          {/* Edit overlay */}
                          <div className="absolute inset-0 bg-[#14b8a6]/70 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <span className="text-white text-[10px] font-bold uppercase tracking-wider">
                              Change
                            </span>
                          </div>
                          <div className="px-2 py-1.5 bg-[#0F172A] flex items-center justify-between gap-1">
                            ...
                            <div className="flex items-center">
                              <button
                                ...
                                className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[#64748B] hover:text-[#14b8a6] transition-colors"
                                title="Edit"
                              >
                              ...
                              <button
                                ...
                                className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[#64748B] hover:text-red-400 transition-colors"
                                title="Remove"
                              >
